...
class LifecycleTransition(Action):
    ...
    def parse(cls, entity: Entity, parser: Parser):
        ...
        node_name_entities = entity.get_attr('lifecycle_node', data_type=List[Entity])
        node_names = []
        for node_entity in node_name_entities:
            n = node_entity.get_attr('name', data_type=str)
            node_names.append(parser.parse_substitution(n))
        kwargs['lifecycle_node_names'] = node_names
        # Probably need to make this parsable through a dict for convenience
        transition_entities = entity.get_attr('transition', data_type=List[Entity])
        transitions = []
        for entity in transition_entities:
            e = entity.get_attr('label', data_type=str)
            transitions.append(parser.parse_substitution(e))
        kwargs['transitions_ids'] = transitions
        return cls, kwargs
    ...
    def execute(
        ...
        for id in subs_transition_ids:
            try:
                ...
                transition_ids.append(self.transition_to_id[id])
